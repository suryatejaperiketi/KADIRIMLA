...
class Predis_Purger extends Purger {
	...
	public function purge_url( $url, $feed = true ) {
		...
		if ( strpos( $_url_purge_base, '*' ) === false ) {
			...
			if ( $status ) {
				$this->log( '- Purge URL | ' . $url2 . '--'. $_url_purge_base );
			} else {
				$this->log( '- Cache Not Found | '. $url2 . '--'. $_url_purge_base, 'ERROR' );
			}
		} else {
			...
			if ( $status ) {
				$this->log( '- Purge Wild Card URL | ' . $url2.'--'. $_url_purge_base . ' | ' . $status . ' url purged' );
			} else {
				$this->log( '- Cache Not Found | ' . $url2 . '--'. $_url_purge_base, 'ERROR' );
			}
		}
